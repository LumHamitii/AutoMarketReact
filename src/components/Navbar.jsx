...
const Navbar = () => {
  ...
  const dropdownOptions = {
    search: ['Used & New Cars', 'Electric Cars', 'Motorcycles'],
    sell: ['Cars', 'Motorcycles', 'Trucks', 'Car valuation'],
    inform: ['Car-Magazine', 'Blog', 'Car manifacturers'],
  };
      ...
      openDropdown === dropdown && (
        <ul className={`absolute ${darkMode ? 'bg-gray-800 text-white' : 'bg-white'} border rounded-md shadow-md mt-2 divide-y divide-gray-200`}>
          {dropdownOptions[dropdown].map((option, index) => (
            <li key={index}>
              <a href="#" className={`block px-4 py-2 ${darkMode ? 'text-white' : 'text-black'}`}>
                {option}
              </a>
            </li>
          ))}
          ...
          <div className='ml-4'>
            <ul className={`hidden md:flex font-semibold ${darkMode ? 'text-white' : 'text-black'}`}>
              <li className='relative'>
                <a href="#" onClick={() => toggleDropdown('search')} className={`flex items-center ${darkMode ? 'text-white' : 'text-black'}`}>
              ...
              <li className='relative'>
                <a href="#" onClick={() => toggleDropdown('sell')} className={`flex items-center ${darkMode ? 'text-white' : 'text-black'}`}>
                  Sell {openDropdown === 'sell' ? <ChevronUpIcon className="w-4 h-4 ml-1" /> : <ChevronDownIcon className="w-4 h-4 ml-1" />}
                </a>
                {renderDropdown('sell')}
              </li>
              <li className='relative'>
                <a href="#" onClick={() => toggleDropdown('inform')} className={`flex items-center ${darkMode ? 'text-white' : 'text-black'}`}>
              ...
              <li><a href=""><UserIcon className={`w-5 ${darkMode ? 'text-white' : 'text-black'}`}/></a></li>
            </ul>
          </div>
        </div>
        ...
        >
          <div className={`w-64 ${darkMode ? 'text-white' : 'text-black'} font-extrabold text-xl`}>
            <ul className='p-4 flex flex-col items-center'>
              ...
              <li><a href=""><UserIcon className={`w-7 ${darkMode ? 'text-white' : 'text-black'}`}/></a></li>
            </ul>
          </div>
